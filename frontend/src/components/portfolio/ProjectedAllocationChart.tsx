...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-[#1e1b2e] border border-gray-200 dark:border-white/15 rounded-xl p-3 text-xs shadow-2xl">
      <p className="font-bold text-gray-900 dark:text-white">{d.sector}</p>
      <p className="text-gray-600 dark:text-white/60 mt-1">{d.percentage}%</p>
    </div>
  );
};
...
function MiniDonut({ data, label }: { data: SectorAllocation[]; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 flex-1">
      <p className="text-xs font-bold text-gray-500 dark:text-white/50 uppercase tracking-widest">{label}</p>
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1.5 justify-center">
        {filtered.map((s, i) => (
          <div key={s.sector} className="flex items-center gap-1">
            ...
            <span className="text-[10px] text-gray-600 dark:text-white/60">{s.sector.length > 8 ? s.sector.slice(0,8)+"…" : s.sector}</span>
            <span className="text-[10px] font-bold text-gray-900 dark:text-white">{s.percentage}%</span>
          </div>
        ))}
      </div>
    </div>
...
export function ProjectedAllocationChart({ current, projected }: Props) {
  return (
    <div className="flex flex-col sm:flex-row gap-6 items-start">
      ...
      <div className="hidden sm:flex flex-col items-center justify-center pt-20 text-gray-300 dark:text-white/30 gap-1">
