  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (waitingClarification && taskId) {
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          { id: Date.now().toString(), role: "assistant", content: "Error: Connection failed." },
        ]);
      ...
      try {
        ...
        if (data.status === "COMPLETED") {
          ...
          if (data.answer) {
             setMessages(prev => {
                const lastMsg = prev[prev.length - 1];
                if (lastMsg.content === data.answer) return prev;
                return [...prev, {
                    id: Date.now().toString(),
                    role: "assistant",
                    content: data.answer,
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 max-w-7xl mx-auto">
          {[
            ...
            <div
              ...
              style={{
                backgroundColor: metric.featured ? "rgba(20, 19, 24, 0.9)" : "rgba(255, 255, 255, 0.7)",
            ...
            <div className="h-96 md:h-[500px] overflow-y-auto p-6 space-y-4">
              ...
              {isProcessing && (
                <div className="flex justify-start">
                  <div className="px-4 py-2 rounded-lg text-sm flex items-center gap-2" style={{ backgroundColor: "rgba(255,255,255,0.5)", color: "#4D4D4D" }}>
            ...
            <div className="border-t border-text-tertiary/10 p-4">
              <form onSubmit={handleSendMessage} className="flex gap-2">
                <input
                  ...
                  style={{ backgroundColor: "rgba(255,255,255,0.5)", color: "#141318" }}
                  disabled={isProcessing}
                />
                <button type="submit" disabled={isProcessing || !input.trim()} className="p-2 rounded-lg transition-all disabled:opacity-50" style={{ backgroundColor: "#141318", color: "#F5F5F9" }}>
